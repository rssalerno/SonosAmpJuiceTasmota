...
var amps = { 
  "Pool": { tasmotaHost: "tasmota3537", stopTimer: null },
  "Deck": { tasmotaHost: "tasmota1137", stopTimer: null }
}
...
if (!justTesting) {
  ...
  var options = { username: 'MQTT_USERNAME',
                  password: 'MQTT_PASSWORD'};
  var client = mqtt.connect('mqtt://MQTT_BROKER_HOST_OR_IP', options)
...
for (var i = 0; i != roomNames.length; i++) {
  ...
  if (!justTesting) {
    ...
    client.publish(amp.tasmotaHost + '/cmnd/POWER', 'off');
  }
}
	...
	*/
	...
	for(var i = 0, length = uuids.length; i < length; i++ ) {
		...
		if (giveItJuice) {
			...
			if (!justTesting) {
			    client.publish(amp.tasmotaHost + '/cmnd/POWER', 'on');
			}
		} else {
...
function stopHandler(roomNameParam) {
  ...
  if (!justTesting) {
    client.publish(amps[roomNameParam].tasmotaHost + '/cmnd/POWER', 'off');
  }
}
